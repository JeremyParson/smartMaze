...
function Maze(lenx, leny, lenz, cell_width) {
  if (lenx) this.lenx = lenx;
  else this.lenx = 9;
  if (leny) this.leny = leny;
  else this.leny = 9;
  if (lenz) this.lenz = lenz;
  else this.lenz = 4;
  if (cell_width) this.cell_width = cell_width;
  else this.cell_width = 10;
  this.maze = [];
    ...
    while (todonum > 0) {
      ...
      while (passBool == 0) {
        ...
        if (
          ...
          if ((maze[x_dx][y_dy][z_dz] & IN_MAZE) == 0) passBool = 1;
        }
      }
  ...
  this.drawMaze = function() {
    ...
    for (z = 0; z < lenz; ++z) {
      ...
      var row_y =
        cell_width + cell_width * (leny + 1) * Math.floor(z / pics_xy);
      for (y = 0; y < leny; ++y) {
        for (x = 0; x < lenx; ++x) {
  ...
  this.drawText = function(x, y, label) {
    ...
    return (
      ...
      '" font-size="' +
      cell_width +
      'px" font-family="LucidaTypewriter Sans">' +
      label +
      ".</text>\n"
    );
  };
